...
async function Navbar() {
  const session = await auth(); // server side 拿到 session的方式
  ...
  return (
    ...
    <nav className="flex w-full flex-col overflow-y-scroll bg-slate-100 pb-10">
      <nav className="sticky top-0 flex flex-col items-start justify-between border-b bg-slate-100 pb-2">
        {/* Chat字樣 */}
        <div className="flex w-fit items-center gap-4 rounded-full p-2 transition-colors duration-300 group-hover:bg-gray-200 lg:pr-4">
          <span className="text-xl max-lg:hidden font-bold">
            Chat
          </span>
        </div>
        ...
        <form
          ...
          action={async () => {
            "use server"; // 記得加這一行 才能進行DB端的操作
            const newDocId = await createDocument(userId);
            revalidatePath("/docs");  // 確定後端刪除了 前端不會再render到
